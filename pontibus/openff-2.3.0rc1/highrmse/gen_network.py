...
def get_water_settings():
    ...
    settings.solvent_forcefield_settings.forcefields = [
        ...
        pathlib.Path("../fb-fit-v6-single-mean-k100-eps-witham1bcc.offxml").read_text(),
        "tip3p.offxml",
    ]
    settings.vacuum_forcefield_settings.forcefields = [
        pathlib.Path("../fb-fit-v6-single-mean-k100-eps-witham1bcc.offxml").read_text(),  # as above
    ]
...
def run(ligands: pathlib.Path):
    ...
    for rdmol in Chem.SDMolSupplier(ligands, removeHs=False):
        ...
        if "mapped_smiles" not in mol.properties:
            # Skip molecules outside the highrmse set
            continue
